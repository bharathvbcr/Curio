...
- **Java `String.hashCode` reimplementation** is REQUIRED wherever Kotlin uses `String.hashCode()` for a deterministic signed Int32 (the store-test `createdAt = id.hashCode().toLong()` default). Call `CurioFormat.javaStringHashCode` — UTF-16 code units, `31 &* h &+ unit`, Int32 overflow. Do not use `String.hashValue` (salted per run) and do not apply `abs` (`Math.abs(Int.MIN_VALUE)` stays negative on the JVM). Category colours come from `CategorySpaces.forCategory`, not a hash palette.
