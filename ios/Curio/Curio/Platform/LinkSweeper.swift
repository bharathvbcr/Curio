...
actor LinkSweeper {
    ...
    func runOneCycle(
        ...
        for (index, entity) in toCheck.enumerated() {
            ...
            if await checkUrlIsBroken(urlString) {
                Self.logger.warning("Dead bookmark link (404/410). Removing.")
                ...
                await firebaseSyncManager.deleteBookmarks(ids: [entity.id])
    ...
    /// Performs a quick HEAD check, returning `true` ONLY for HTTP 404/410. Any transport error
    /// (connection refused / unresolved host / offline) is TRANSIENT and treated as intact (`false`)
    /// so a temporary network blip never deletes the user's bookmark. Port of `checkUrlIsBroken`.
    ///
