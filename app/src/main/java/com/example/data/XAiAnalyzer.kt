...
/**
 * Outcome of probing the configured xAI key against GET /v1/api-key. Distinguishes "the key is
 * wrong" ([Invalid]) from "we couldn't ask" ([Unreachable]) so the Settings UI never tells a user
 * on a flaky connection that their key is bad.
 */
sealed interface KeyVerification {
    data object Valid : KeyVerification
    data class Invalid(val reason: String) : KeyVerification
    data class Unreachable(val message: String) : KeyVerification
}
...
Write all field values in English, regardless of the source content's language.
...
class XAiAnalyzer(
    ...
    /**
     * Probes the configured key against xAI's key-introspection endpoint (consumes no tokens).
     * Backs the live/invalid feedback in Settings → xAI API Key.
     */
    suspend fun verifyKey(): KeyVerification = withContext(Dispatchers.IO) {
        val apiKey = XaiKeyStore.resolve()
        if (!XaiKeyStore.isConfigured()) {
            return@withContext KeyVerification.Invalid("No key configured")
        }
        try {
            val info = xAiApi.getApiKeyInfo("Bearer $apiKey")
            when {
                info.apiKeyBlocked == true ->
                    KeyVerification.Invalid("xAI has blocked this key")
                info.apiKeyDisabled == true ->
                    KeyVerification.Invalid("Key is disabled — re-enable it at console.x.ai")
                info.teamBlocked == true ->
                    KeyVerification.Invalid("Your xAI team is blocked — check billing at console.x.ai")
                else -> KeyVerification.Valid
            }
        } catch (e: retrofit2.HttpException) {
            when (e.code()) {
                401, 403 -> KeyVerification.Invalid("xAI rejected the key (HTTP ${e.code()})")
                else -> KeyVerification.Unreachable("xAI returned HTTP ${e.code()}")
            }
        } catch (e: java.io.IOException) {
            KeyVerification.Unreachable("Network error — couldn't reach xAI")
        } catch (e: Exception) {
            if (e is kotlinx.coroutines.CancellationException) throw e
            KeyVerification.Unreachable(e.message ?: "Unknown error")
        }
    }
    ...
    suspend fun deepAnalyzeBookmark(
        ...
        val request = XAiRequest(
            ...
            messages = listOf(
                XAiMessage(role = "system", content = "You are an AI/ML research analyst. Return only valid JSON. Write all field values in English, regardless of the source content's language."),
                XAiMessage(role = "user", content = prompt)
            ),
            temperature = 0.3f,
    ...
    suspend fun generateChatResponse(
        ...
        try {
            ...
            ChatResponse("Connection failed: ${e.localizedMessage ?: "Unknown networking error"}. Check your xAI API key in Settings.")
        }
    }
    ...
    suspend fun generateWeeklyDigest(itemsBlock: String, itemCount: Int): String = withContext(Dispatchers.IO) {
        ...
        val request = XAiRequest(
            model = GrokModels.DEEP_ANALYSIS,
            messages = listOf(
                XAiMessage(role = "system", content = "You are a research librarian writing a weekly digest. Output only markdown. Write in English, regardless of the language of the items."),
                XAiMessage(role = "user", content = prompt)
            ),
            temperature = 0.5f,
